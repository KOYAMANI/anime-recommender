...
describe('APIService', () => {
    ...
    test('getRecommendation returns expected data', async () => {
        ...
        expect(fetchMock.mock.calls[0][0]).toEqual(
            `${process.env.REACT_APP_API_URL}api/v1/anime/recommend/undefined`
        )
    })
    ...
    test('getRecommendation returns expected data with user id', async () => {
        ...
        const res = await apiService.getRecommendation(
    ...
    test('getSuggestion returns expected data', async () => {
        fetchMock.mockResponseOnce(JSON.stringify(getSuggestionData))
    ...
    test('getUserHistory returns expected data', async () => {
        fetchMock.mockResponseOnce(JSON.stringify(getUserHistoryData))

        const res = await apiService.getUserHistory(userId)
